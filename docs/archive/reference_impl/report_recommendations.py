...
def build_recommendations(
    *,
    ...
    for crash_class, count in crash_counts.most_common():
        if count >= 3:
            notes.append(f"Reliability first: {count} recent `{crash_class}` failures suggest suppressing similar proposals until fixed.")
    ...
    if near_miss_count >= 3:
        notes.append("There are several near-misses; schedule targeted ablations and combinations before opening a new code-lane initiative.")

    if confirm_promotions == 0:
        notes.append("Few candidates are reaching confirm; widen scout novelty slightly or relax exploit locality to avoid overfitting one region.")
