...
class ExportCodeProposalTests(unittest.TestCase):
    ...
    def test_export_code_proposal_writes_self_contained_pack(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse((pack_root / "context" / "campaign.json").exists())
            self.assertFalse((pack_root / "context" / "best_comparator.json").exists())
            self.assertFalse((pack_root / "context" / "parent_runs.json").exists())
